...
class ConfigurationTest {
    @Test
    void openApiConfigCreatesOrderApiMetadata() {
        assertThat(new OpenApiConfig().openAPI().getInfo().getTitle()).isEqualTo("Order Service API");
    }
    ...
    void kafkaRetryConfigRoutesFailuresToDeadLetterTopic() {
        var partition = KafkaRetryConfig.dltPartition(new ConsumerRecord<>("payments", 1, 0L, "key", "value"),
